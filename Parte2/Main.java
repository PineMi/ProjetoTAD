...
public class Main {
        ...
        try {
            if (delParts.length == 1) {
                ...
                if (found) {
                    ...
                    System.out.println("Linha " + lineNumber + " removida com sucesso.");
                } else {
                ...
                while (pAnda != null) {
                    ...
                    if (currentLineNumber >= startLine && currentLineNumber <= endLine) {
                        ...
                        System.out.println("Linha " + currentLineNumber + " removida com sucesso.");
                    } else {
                        pAnt = pAnda;
                        pAnda = pAnda.getProx();
                    }
    ...
    private static void exit() {
        if (unsavedChanges) {
            System.out.print("Existem alterações não salvas. Deseja sair sem salvar? (S/N): ");
            String response = scanner.nextLine();
            if (response.equalsIgnoreCase("N")) {
                System.out.println("Por favor, salve suas alterações antes de sair.");
